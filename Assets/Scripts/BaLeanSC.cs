...
public class BaLeanSC : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (PlayerPrefs.GetInt("Choice1") == 2)
        {
            ...
            //components
            if (Input.GetKeyDown(KeyCode.R) && canThrow == true && Ani.GetBool("isCrouching") == false && movement.isGrounded() && movement.Ani.GetBool("isThrow") == false && movement.Punching == false)
            {
                canThrow = false;
